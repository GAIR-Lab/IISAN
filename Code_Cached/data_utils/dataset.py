...
class Build_MM_Dataset(Dataset):
    def __init__(self, u2seq, item_content, item_num, max_seq_len, db_path, item_id_to_keys, resize, neg_sampling_list,stored_vector_path):
        self.u2seq = u2seq
    ...
    def __getitem__(self, user_id):
        ...
        for i in range(seq_Len):
            ...
            bert_output_loaded = load_output(self.stored_vector_path+'/bert_outputs', item_id, prefix='bert')


            sample_items_text.append(bert_output_loaded)            
            vit_output_loaded = load_output(self.stored_vector_path+'/vit_outputs', item_id, prefix='vit')
...
class Build_MM_EMBED_Eval_Dataset_Cached(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if item_id!=0:
            ...
            return torch.zeros((13, 768)),torch.zeros((13, 768))
        item_key = item_key.decode('utf-8')
        bert_output_loaded = load_output(self.stored_vector_path+'/bert_outputs', item_key, prefix='bert')    
        vit_output_loaded = load_output(self.stored_vector_path+'/vit_outputs', item_key, prefix='vit')
