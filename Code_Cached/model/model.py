...
class IISANAdaptedMModel(nn.Module):
    ...
    def forward(self,sample_items_images, sample_items_text):
        hidden_states_cv = [sample_items_images[:, :, i, :].view(-1, 768) if sample_items_images.dim() == 4 else sample_items_images[:, i, :].view(-1, 768) for i in range(13)]
        hidden_states_text = [sample_items_text[:, :, i, :].view(-1, 768) if sample_items_text.dim() == 4 else sample_items_text[:, i, :].view(-1, 768) for i in range(13)]
