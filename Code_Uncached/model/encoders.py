...
class MM_Encoder(torch.nn.Module):
    def __init__(self,args,image_net,bert_model):
        ...
        if "clip-vit" in args.CV_model_load:
            self.cv_encoder = ClipVit_Encoder(image_net=image_net,item_dim=args.embedding_dim)
...
class Vit_Encoder(torch.nn.Module):
    ...
    def forward(self, item_content):
        ...
        return self.activate(outputs[0]),outputs.hidden_states
...
class Text_Encoder(torch.nn.Module):
    ...
    def forward(self, text):
        ...
        outputs = self.bert_model(input_ids=text_ids, attention_mask=text_attmask,output_hidden_states=True)
...
class Bert_Encoder(torch.nn.Module):
    def __init__(self, args, bert_model):
        ...
        self.text_encoders = nn.ModuleDict({
            'title': Text_Encoder(bert_model,args, args.embedding_dim, args.word_embedding_dim)
        })
