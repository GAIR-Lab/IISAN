...
def train(args, use_modal, local_rank):
    if use_modal:
        #--------------------------------- Image ---------------------------------
        if 'clip-vit' in args.CV_model_load:
            clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
            cv_model = clip_model.vision_model
            del clip_model
        ...
        for index, (name, param) in enumerate(cv_model.named_parameters()):
            if index < args.freeze_paras_before or "post_layernorm" in name:
                param.requires_grad = False
        
        #--------------------------------- Text ---------------------------------
        pooler_para=[]
        if 'deberta' in args.bert_model_load:
            Log_file.info('load deberta model...')
            bert_model_load = args.bert_model_load
            tokenizer = AutoTokenizer.from_pretrained(bert_model_load)
            bert_model = DebertaV2Model.from_pretrained(bert_model_load)
        else:
    ...
    if 'None' in args.adding_adapter_to:
        ...
        if "bitfit" in args.adapter_type:
            find_unused = True
            for index, (name, param) in enumerate(model.named_parameters()):
                if any(["user" in name, "cv_proj" in name, "classifier" in name, "title.fc" in name ]) or all(["user" not in name, "encoder" not in name]):
                    param.requires_grad = True
                if "mm_encoder" in name and "bias" in name:
                    param.requires_grad = True
            ...
            for index, layer_module in enumerate(
                ...
                layer_module.attention.attention.query = lora.Linear(args.word_embedding_dim, args.word_embedding_dim, r=args.bert_adapter_down_size).to(local_rank)
                layer_module.attention.attention.value = lora.Linear(args.word_embedding_dim, args.word_embedding_dim, r=args.bert_adapter_down_size).to(local_rank)
            ...
            for index, (name, param) in enumerate(model.named_parameters()):
                if "intra" in args.modality:
                    if any(["user" in name,"cv_proj" in name, "classifier" in name, "title.fc" in name, "lm_head" in name]) or all(["user" not in name, "encoder" not in name]):
                        param.requires_grad = True
                else:
                    if any(["user" in name]) or all(["user" not in name, "encoder" not in name]):
                        param.requires_grad = True
            ...
            if "mae" not in args.CV_model_load:
                ...
                for index, layer_module in enumerate(
                    ...
                    layer_module.output = add_adapter_to_vit_output(layer_module.output, args).to(
                        local_rank)
                # adding adapters to the SASRec model the same to cv encoder
                for index, (name, param) in enumerate(model.named_parameters()):
                    if any(["user" in name, "classifier" in name, "title.fc" in name]) or all(["user" not in name, "encoder" not in name]):
                        param.requires_grad = True
    ...
    if use_modal:
        ...
        if "None" not in args.adding_adapter_to:
            ...
            len(list(model.module.mm_encoder.cv_encoder.image_net.parameters())),
    ...
    for ep in range(args.epoch):
        ...
        for data in train_dl:
            ...
            if use_modal:
                sample_items_image = sample_items_image.view(-1, 3, args.CV_resize, args.CV_resize)
                sample_items_text = sample_items_text.view(-1, sample_items_text.size(
                    -1))
            else:
        ...
        if not need_break:
            ...
            if max_eval_value > max_hit10 or max_hit10 == 0 or ep % 10 == 0:
                ...
                if use_modal and dist.get_rank() == 0:
                    run_eval_test(model, item_id_to_keys,item_content, users_history_for_test, users_test, args.batch_size, item_num,
                          use_modal,
