...
def train(args, use_modal, local_rank):
    ...
    if use_modal:
        train_dataset = Build_MM_Dataset(u2seq=users_train, item_content=item_content, item_num=item_num, max_seq_len=args.max_seq_len,
                                           ...
                                           neg_sampling_list=neg_sampling_list,stored_vector_path=args.stored_vector_path,cached_image_model=args.cached_image_model,cached_text_model=args.cached_text_model,args=args)
    else:
    ...
    print("Text Gates:")
    for index,gate_params in enumerate(model.mm_encoder.side_gate_params_text):
        gate = torch.sigmoid(gate_params / 0.1)
        print(f"number {index}:", gate)
    
    print("CV Gates:")
    for index,gate_params in enumerate(model.mm_encoder.side_gate_params_cv):
        gate = torch.sigmoid(gate_params / 0.1)
        print(f"number {index}:", gate)
    
    print("MM Gates:")
    for index,gate_params in enumerate(model.mm_encoder.side_gate_params_mm):
        gate = torch.sigmoid(gate_params / 0.1)
        print(f"number {index}:", gate)
    ...
    for ep in range(args.epoch):
        ...
        for data in train_dl:
            ...
            if use_modal:
                sample_items_image = sample_items_image.view(-1, 11, args.image_layers+1, args.image_embedding_dim)
                sample_items_text = sample_items_text.view(-1, 11, args.text_layers+1, args.text_embedding_dim)
            else:
